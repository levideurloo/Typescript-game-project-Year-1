    ...
    private loadCharacters() {
        ...
        [boy_1, boy_2, girl_1, girl_2].forEach(function (element) {
            element.setInteractive().on('pointerdown', function (this: Phaser.GameObjects.Image) {
                ...
                //destroy text
                const selectedCharacterText = (this.scene as GameOverScene).selectedCharacterText;
                ...
                //set selected text
                (this.scene as GameOverScene).selectedCharacterText = this.scene.add.text(this.x - this.displayWidth / 2 + 20, (this.y + this.displayHeight - 35), 'ˆ').setColor('#baff80').setFontSize(32);
            });
        }, this);
